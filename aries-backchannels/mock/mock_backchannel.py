...
from mock.routes.agent_routes import routes as agent_routes
from mock.routes.mediation_routes import (
...
class MockAgentBackchannel(AgentBackchannel):
    def __init__(
        self,
        ident: str,
        ...
        # get mock version if available
        self.mock_version = None
        try:
            with open("./mock-version.txt", "r") as file:
                self.mock_version = file.readline()
        except Exception:
            # ignore errors
            pass

        # set the mock AIP version, defaulting to AIP10
    ...
    def get_mock_version_as_float(self):
        ...
        if not self.mock_version or 0 == len(self.mock_version):
            return 0.0

        descriptiveTrailer = "-"
        comparibleVersion = self.mock_version
    ...
    def get_agent_args(self):
        ...
        if self.get_mock_version_as_float() > 56:
        ...
        # This code for Tails Server is included here because mock does not support the env var directly yet.
        ...
        # This code for log level is included here because mock does not support the env var directly yet.
        # when it does (and there is talk of supporting YAML) then this code can be removed.
        if os.getenv("LOG_LEVEL") is not None:
            result.append(("--log-level", os.getenv("LOG_LEVEL")))

        # mock supports a config.yaml file to pass in arguments. This env var point to such a file.
        ...
        # result.append(("--trace", "--trace-target", "log", "--trace-tag", "mock.events", "--trace-label", "mock",))
    ...
    async def make_agent_POST_request(
        ...
        if command.topic == "connection":
            ...
            if operation == "create-invitation":
                ...
                if self.auto_accept_requests and operation == "accept-request":
                    resp_status = 200
                    resp_text = "Mock agent in auto accept request mode. accept-request operation not called."
                else:
            ...
            if (
                ...
                resp_status = 200
                resp_text = '{"message": "Mock agent in auto respond mode for proposal. send-offer operation not called."}'
                return (resp_status, resp_text)
            elif self.auto_respond_credential_offer and operation == "send-request":
                resp_status = 200
                resp_text = '{"message": "Mock agent in auto respond mode for offer. send-request operation not called."}'
                return (resp_status, resp_text)
            elif self.auto_respond_credential_request and operation == "issue":
                resp_status = 200
                resp_text = '{"message": "Mock agent in auto respond mode for request. issue operation not called."}'
                return (resp_status, resp_text)
            else:
                if command.record_id is None:
                    agent_operation = mock_topic + operation
                else:
                    if (
                        ...
                        agent_operation = (
                            f"{mock_topic}records/{cred_ex_id}/{operation}"
                        )
                        ...
                        if (
                            ...
                            if not await self.expected_agent_state(
                                f"{mock_topic}records/{cred_ex_id}",
                                "request_received",
                                wait_time=60.0,
                            ):
                                raise Exception(
        ...
        # Handle proof v2 POST operations
            ...
            # set the mockversion to master since work to set it is not complete. Remove when master report proper version
            # self.mock_version = "0.5.5-RC"
            operation = command.operation
            (
                agent_operation,
                admin_data,
            ) = await self.get_agent_operation_mock_version_based(command)
            ...
            if (
                ...
                resp_status = 200
                resp_text = '{"message": "Mock agent in auto respond mode for presentation proposal. send-request operation not called."}'
                log_msg(
                    "Mock agent in auto respond mode for presentation proposal. send-request operation not called."
                )
                return (resp_status, resp_text)
            elif (
                ...
                resp_status = 200
                resp_text = '{"message": "Mock agent in auto respond mode for presentation request. send-presentation operation not called."}'
                log_msg(
                    "Mock agent in auto respond mode for presentation request. send-presentation operation not called."
                )
                return (resp_status, resp_text)
            else:
                ...
                if data is not None:
                    # Format the message data that came from the test, to what the Mock admin api expects.
        ...
        if (
            ...
            if operation == "prepare-json-ld":
                ...
                if did:
                    ...
                    # Mock doesn't include did:sov prefix for sov dids
            ...
            # Map AATH filter keys to Mock filter keys
            ...
            if operation == "store":
                ...
                # Return less Mock specific credential identifier key
    ...
    async def make_agent_GET_request(
        ...
        if command.topic == "version":
            if self.mock_version is not None:
                status = 200
                status_msg = self.mock_version
            else:
                status = 404
    ...
    def get_process_args(self, bin_path: Optional[str] = None) -> List[str]:
        # TODO mock needs to be in the path so no need to give it a cmd_path
        cmd_path = "mock"
        if bin_path is None:
            bin_path = DEFAULT_BIN_PATH
        if bin_path:
            cmd_path = os.path.join(bin_path, cmd_path)
        print("Location of Mock: " + cmd_path)
        if self.get_mock_version_as_float() > 56:
    ...
    async def detect_process(self):
        ...
        try:
            ...
            if ok:
                self.mock_version = status["version"]
                print(
                    "Mock Backchannel running with Mock version:",
                    self.mock_version,
                )
    ...
    def agent_state_translation(self, topic: str, data: str):
        ...
        # the following is what the tests/rfc expect vs what mock communicates
        # Connection Protocol:
        # Tests/RFC         |   Mock
        # invited           |   invitation
        # requested         |   request
    ...
    async def get_agent_operation_mock_version_based(
        self, command: BackchannelCommand
    ) -> Tuple[str, Optional[Mapping[str, Any]]]:
        # Admin api calls may change with mock releases. For example revocation related calls change
        # between 0.5.4 and 0.5.5. To be able to handle this the backchannel is made aware of the mock version
        # and constructs the calls based off that version

        # construct some number to compare to with > or < instead of listing out the version number
        comparibleVersion = self.get_mock_version_as_float()
        ...
        if topic == "revocation":
            if operation == "revoke":
                if comparibleVersion > 54:
                    ...
                    if (
                        data is not None
                    ):  # Data should be included with 0.5.4 or lower mock. Then it takes them as inline parameters.
                        cred_rev_id = data["cred_rev_id"]
                        rev_reg_id = data["rev_registry_id"]
                        publish = data["publish_immediately"]
                        agent_operation = (
...
async def main(start_port: int, show_timing: bool = False, interactive: bool = True):
    ...
    try:
        ...
        # start mock agent sub-process and listen for web hooks
